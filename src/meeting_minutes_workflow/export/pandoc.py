...
def export_run_markdown_to_docx(run_folder: Path, *, runner: Runner | None = None) -> None:
    ...
    for markdown_filename in EXPECTED_MARKDOWN_OUTPUTS:
        ...
        export_markdown_to_docx(markdown_file, docx_file, reference_doc=reference_doc, runner=runner)
...
def _is_actions_table_header(lines: list[str], index: int) -> bool:
    if index + 1 >= len(lines):
        return False
    return lines[index].strip() == "| Action | Owner | Due date | Review note |" and lines[index + 1].strip() == "|---|---|---|---|"
